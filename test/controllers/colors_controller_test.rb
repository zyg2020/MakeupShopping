...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { hex_value: @color.hex_value, name: @color.name, product_id: @color.product_id } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { hex_value: @color.hex_value, name: @color.name, product_id: @color.product_id } }
